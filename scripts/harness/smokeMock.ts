...
export interface RunMockSmokeOptions {
  appRoot: string;
...
export const runMockSmoke = async ({
  appRoot,
  contract,
  ...
  userDataDir
}: RunMockSmokeOptions): Promise<number> => {
  ...
  return result.status === "passed"
    ...
    : resolveFailureExitCode(result.failureCategory, contract);
};

const resolveFailureExitCode = (
  failureCategory: Awaited<ReturnType<typeof runElectronHarness>>["failureCategory"],
  contract: NodeHarnessRuntimeContract
): number => {
  switch (failureCategory) {
    case "renderer-never-attached":
      return contract.exitCodes.attachTimeout;
    case "bootstrap-timeout":
      return contract.exitCodes.bootstrapTimeout;
    case "renderer-crash":
      return contract.exitCodes.appCrash;
    case "console-error":
      return contract.exitCodes.assertionFailure;
    default:
      return contract.exitCodes.unknownFailure;
  }
};
...
const resolveCliEntryScript = (argv: string[], cwd: string): string => {
  ...
  return resolve(cwd, "test/harness/fixtures/electron-smoke-main.cjs");
};
...
if (process.argv[1] && import.meta.url === new URL(process.argv[1], "file:").href) {
  ...
  const exitCode = await runMockSmoke({
    appRoot:
