...
import { runChronologyScenario } from "./chronologyScenario";

const tempDirs: string[] = [];

const createTempDir = async (): Promise<string> => {
  const dir = await mkdtemp(join(tmpdir(), "codex-chronology-scenario-"));
  tempDirs.push(dir);
  return dir;
};
...
describe("chronology app scenario", () => {
  it(
    "passes the chronology UI scenario against the actual Electron app",
    async () => {
      const userDataDir = await createTempDir();
      const result = await runElectronHarness({
        appRoot: resolve(process.cwd()),
        userDataDir,
        contract: makeContract("t11-chronology-scenario"),
